...
{
    int i;
    for (i = 0; i < w->numpoints; i++) printf("(%5.1f, %5.1f, %5.1f)\n", w->p[i][0], w->p[i][1], w->p[i][2]);
}
...
{
    if (*(unsigned *)w == 0xdeaddead) Com_Error(ERR_FATAL, "FreeWinding: freed a freed winding");
...
winding_t *BaseWindingForPlane(vec3_t normal, vec_t dist)
{
    ...
    if (x == -1) Com_Error(ERR_DROP, "BaseWindingForPlane: no axis found");
...
void ClipWindingEpsilon(winding_t *in, vec3_t normal, vec_t dist, vec_t epsilon, winding_t **front, winding_t **back)
{
    ...
    *back = b = AllocWinding(maxpts);
    ...
    if (f->numpoints > maxpts || b->numpoints > maxpts) Com_Error(ERR_DROP, "ClipWinding: points exceeded estimate");
...
void ChopWindingInPlace(winding_t **inout, vec3_t normal, vec_t dist, vec_t epsilon)
{
    ...
    if (f->numpoints > maxpts) Com_Error(ERR_DROP, "ClipWinding: points exceeded estimate");
    if (f->numpoints > MAX_POINTS_ON_WINDING) Com_Error(ERR_DROP, "ClipWinding: MAX_POINTS_ON_WINDING");
...
void CheckWinding(winding_t *w)
{
    ...
    if (w->numpoints < 3) Com_Error(ERR_DROP, "CheckWinding: %i points", w->numpoints);

    area = WindingArea(w);
    if (area < 1) Com_Error(ERR_DROP, "CheckWinding: %f area", area);
    ...
    for (i = 0; i < w->numpoints; i++)
    {
        ...
        if (d < -ON_EPSILON || d > ON_EPSILON) Com_Error(ERR_DROP, "CheckWinding: point off plane");

        // check the edge isnt degenerate
        p2 = w->p[j];
        VectorSubtract(p2, p1, dir);

        if (VectorLength(dir) < ON_EPSILON) Com_Error(ERR_DROP, "CheckWinding: degenerate edge");
        ...
        for (j = 0; j < w->numpoints; j++)
        {
            ...
            if (d > edgedist) Com_Error(ERR_DROP, "CheckWinding: non-convex");
        }
    }
}
