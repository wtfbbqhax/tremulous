...
static void CM_ProximityToBrush(traceWork_t *tw, cbrush_t *brush)
{
    ...
    if (tw2.trace.fraction == 1.0f && !tw2.trace.allsolid && !tw2.trace.startsolid)
    {
        ...
        if (fraction < tw->trace.lateralFraction) tw->trace.lateralFraction = fraction;
    }
    else
...
// trace line against all patches in the leaf
    ...
    if (tw->testLateralCollision && tw->trace.fraction < 1.0f)
    {
        for (k = 0; k < leaf->numLeafBrushes; k++)
        {
            ...
            if (!tw->trace.lateralFraction) return;
        }
...
================
*/
void CM_TraceThroughVerticalCylinder(
    traceWork_t *tw, vec3_t origin, float radius, float halfheight, vec3_t start, vec3_t end)
{
...
void CM_Trace(trace_t *results, const vec3_t start, const vec3_t end, vec3_t mins, vec3_t maxs, clipHandle_t model,
    const vec3_t origin, int brushmask, traceType_t type, sphere_t *sphere)
{
    int i;
    traceWork_t tw;
    vec3_t offset;
...
==================
*/
void CM_BiSphereTrace(
    trace_t *results, const vec3_t start, const vec3_t end, float startRad, float endRad, clipHandle_t model, int mask)
{
    int i;
    traceWork_t tw;
    ...
    {
        for (i = 0; i < 3; i++) tw.trace.endpos[i] = start[i] + tw.trace.fraction * (end[i] - start[i]);
    }
...
==================
*/
void CM_TransformedBiSphereTrace(trace_t *results, const vec3_t start, const vec3_t end, float startRad, float endRad,
