...
static void EmitMovEAXStack(vm_t *vm, int andit)
{
    if (!jlabel)
    {
        ...
        else if (pop1 != OP_DIVI && pop1 != OP_DIVU && pop1 != OP_MULI && pop1 != OP_MULU && pop1 != OP_STORE4 &&
...
void EmitMovEDXStack(vm_t *vm, int andit)
{
    ...
    if (andit) MASK_REG("E2", andit);  // and edx, 0x12345678
}
...
#define JUSED(x)                                                                             \
    ...
    {                                                                                        \
        ...
        {                                                                                    \
            VMFREE_BUFFERS();                                                                \
            Com_Error(ERR_DROP, "VM_CompileX86: jump target out of range at offset %d", pc); \
        }                                                                                    \
        jused[x] = 1;                                                                        \
    } while (0)
...
static void DoSyscall(void)
{
    ...
    if (vm_syscallNum < 0)
    {
        ...
        for (int i = 1; i < ARRAY_LEN(args); i++) args[i] = data[i];
    ...
    else
    {
        switch (vm_syscallNum)
        {
            case VM_JMP_VIOLATION:
                ...
                break;
            case VM_BLOCK_COPY:
                if (vm_opStackOfs < 1) Com_Error(ERR_DROP, "VM_BLOCK_COPY failed due to corrupted opStack");
...
void VM_Compile(vm_t *vm, vmHeader_t *header)
{
        ...
        while (instruction < header->instructionCount)
        {
            ...
            switch (op)
            {
                ...
                case OP_ENTER:
                    ...
                    break;
                case OP_CONST:
                    if (ConstOptimize(vm, callProcOfsSyscall)) break;
                    ...
                    Emit4(lastConst);
                    if (code[pc] == OP_JUMP) JUSED(lastConst);
    ...
    if (vm->codeBase == MAP_FAILED) Com_Error(ERR_FATAL, "VM_CompileX86: can't mmap memory");
#elif _WIN32
    // allocate memory with EXECUTE permissions under windows.
    vm->codeBase = (byte *)VirtualAlloc(NULL, compiledOfs, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    if (!vm->codeBase) Com_Error(ERR_FATAL, "VM_CompileX86: VirtualAlloc failed");
#else
    vm->codeBase = malloc(compiledOfs);
    if (!vm->codeBase) Com_Error(ERR_FATAL, "VM_CompileX86: malloc failed");
#endif
...
int VM_CallCompiled(vm_t *vm, int *args)
{
    ...
    for (arg = 0; arg < MAX_VMMAIN_ARGS; arg++) *(int *)&image[programStack + 8 + arg * 4] = args[arg];
    ...
    *opStack = 0xDEADBEEF;
    ...
    if (opStackOfs != 1 || *opStack != 0xDEADBEEF) { Com_Error(ERR_DROP, "opStack corrupted in compiled code"); }
