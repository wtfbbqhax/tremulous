...
void CMod_LoadSubmodels(lump_t *l)
{
    ...
    if (l->filelen % sizeof(*in)) Com_Error(ERR_DROP, "CMod_LoadSubmodels: funny lump size");
    count = l->filelen / sizeof(*in);
...
void CMod_LoadNodes(lump_t *l)
{
    ...
    if (l->filelen % sizeof(*in)) Com_Error(ERR_DROP, "MOD_LoadBmodel: funny lump size");
    count = l->filelen / sizeof(*in);
...
static bool CMod_AddEdgeToBrush(const vec3_t p0, const vec3_t p1, cbrushedge_t *edges, int *numEdges)
{
    ...
    if (!edges || !numEdges) return false;

    for (i = 0; i < *numEdges; i++)
    {
        if (CMod_BrushEdgesAreTheSame(p0, p1, edges[i].p0, edges[i].p1)) return false;
    }
    ...
    for (i = 0; i < cm.numBrushes; i++)
    {
        ...
        for (j = 0; j < brush->numsides; j++)
        {
            ...
            for (k = 0; k < brush->numsides && w != NULL; k++)
            {
                ...
                if (chopSide == side) continue;

                if (chopSide->planeNum == (side->planeNum ^ 1)) continue;  // back side clipaway
...
void CMod_LoadPatches(lump_t *surfs, lump_t *verts)
{
    ...
    if (surfs->filelen % sizeof(*in)) Com_Error(ERR_DROP, "MOD_LoadBmodel: funny lump size");
    cm.numSurfaces = count = surfs->filelen / sizeof(*in);
    cm.surfaces = (cPatch_t **)Hunk_Alloc(cm.numSurfaces * sizeof(cm.surfaces[0]), h_high);

    dv = (drawVert_t *)(cmod_base + verts->fileofs);
    if (verts->filelen % sizeof(*dv)) Com_Error(ERR_DROP, "MOD_LoadBmodel: funny lump size");
...
//
    ...
    *checksum = last_checksum;
    ...
    {
        Com_Error(
            ERR_DROP, "CM_LoadMap: %s has wrong version number (%i should be %i)", name, header.version, BSP_VERSION);
    }
