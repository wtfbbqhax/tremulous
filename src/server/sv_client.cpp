...
void SV_DirectConnect( netadr_t from ) {
	...
	for (i=0,cl=svs.clients ; i < sv_maxclients->integer ; i++,cl++)
    {
			...
			&& (cl->netchan.qport == qport || from.port == cl->netchan.remoteAddress.port) )
        {
            ...
            //			sv.gvm->Call( GAME_CLIENT_DISCONNECT, newcl - svs.clients );
			goto gotnewcl;
		}
	}
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	sv.gvm->Call( GAME_CLIENT_DISCONNECT, drop - svs.clients );
...
void SV_ClientEnterWorld( client_t *client, usercmd_t *cmd ) {
	...
	sv.gvm->Call( GAME_CLIENT_BEGIN, client - svs.clients );
}
...
static void SV_UpdateUserinfo_f( client_t *cl ) {
	...
	sv.gvm->Call( GAME_CLIENT_USERINFO_CHANGED, cl - svs.clients );
}
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, bool clientOK ) {
	...
	if (clientOK) {
		...
		if (!u->name && sv.state == SS_GAME && (cl->state == CS_ACTIVE || cl->state == CS_PRIMED)) {
			sv.gvm->Call( GAME_CLIENT_COMMAND, cl - svs.clients );
		}
	}
	else if (!bProcessed)
...
void SV_ClientThink (client_t *cl, usercmd_t *cmd) {
	...
	sv.gvm->Call( GAME_CLIENT_THINK, cl - svs.clients );
}
