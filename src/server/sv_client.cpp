...
void SV_GetChallenge(netadr_t from)
{
    ...
    if (sv_rsaAuth->integer)
    {
        NET_OutOfBandPrint(NS_SERVER, challenge->adr, "challengeResponse %d %d %d %s", challenge->challenge,
            clientChallenge, PROTOCOL_VERSION, challenge->challenge2);
    }
    else
    {
        NET_OutOfBandPrint(NS_SERVER, challenge->adr, "challengeResponse %d %d %d", challenge->challenge,
            clientChallenge, PROTOCOL_VERSION);
    }
}
...
int SV_WriteDownloadToClient(client_t *cl, msg_t *msg)
{
    ...
    if (!cl->download)
    {
        ...
        if (!(sv_allowDownload->integer & DLF_ENABLE) || (sv_allowDownload->integer & DLF_NO_UDP) || unreferenced ||
...
static ucmd_t ucmds[] = {{"userinfo", SV_UpdateUserinfo_f}, {"disconnect", SV_Disconnect_f}, {"cp", SV_VerifyPaks_f},
    {"vdr", SV_ResetPureClient_f}, {"download", SV_BeginDownload_f}, {"nextdl", SV_NextDownload_f},
    {"stopdl", SV_StopDownload_f}, {"donedl", SV_DoneDownload_f},
...
static void SV_UserMove(client_t *cl, msg_t *msg, bool delta)
{
    ...
    key ^= MSG_HashKey(
        cl->netchan.alternateProtocol, cl->reliableCommands[cl->reliableAcknowledge & (MAX_RELIABLE_COMMANDS - 1)], 32);
