...
void QDECL SV_SendServerCommand(client_t *cl, const char *fmt, ...)
{
    ...
    {
        Com_Printf(
            "SV_SendServerCommand( %ld, %.20s... ) length %ld > 1022, dropping to avoid server buffer overflow.\n",
            ...
            if (sv_masters[a][i]->modified || (adrs[a][i][0].type == NA_BAD && adrs[a][i][1].type == NA_BAD))
            {
                ...
                if (netenabled & NET_ENABLEV4)
                {
                    ...
                    if (res == 2)
                    {
                        ...
                        adrs[a][i][0].port =
                            BigShort(a == 2 ? ALT2PORT_MASTER : a == 1 ? ALT1PORT_MASTER : PORT_MASTER);
                    }
...
static leakyBucket_t *SVC_BucketForAddress(netadr_t address, int burst, int period)
{
    ...
    for (int i = 0; i < MAX_BUCKETS; i++)
    {
        ...
        if (bucket->type == NA_BAD)
        {
            ...
            {
                case NA_IP:
                    ::memcpy(bucket->ipv._4, address.ip, 4);
                    break;
                case NA_IP6:
                    ::memcpy(bucket->ipv._6, address.ip6, 16);
                    break;
                default:
                    break;
            }
...
void SVC_Info(netadr_t from)
{
    ...
    Info_SetValueForKey(infostring, "protocol",
        va("%i", from.alternateProtocol == 2 ? 69 : from.alternateProtocol == 1 ? 70 : PROTOCOL_VERSION));
